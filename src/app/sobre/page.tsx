...
export default function SobrePage() {
  return (
    <div style={{ background: C.bg, minHeight: '100vh', color: C.white }}>
      ...
      {/* Header */}
      <section style={{ padding: 'clamp(120px, 15vh, 160px) clamp(24px, 5vw, 64px) clamp(60px, 8vw, 100px)', textAlign: 'center', position: 'relative' }}>
        ...
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.15 }} style={{ fontFamily: 'var(--font-cormorant)', fontSize: 'clamp(42px, 6vw, 80px)', fontWeight: 300, lineHeight: 1.1, color: C.white }}>
          Víctor Mago
        </motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.35 }} style={{ fontFamily: 'var(--font-cormorant)', fontSize: 'clamp(18px, 2.5vw, 26px)', fontStyle: 'italic', color: C.copper, marginTop: 8 }}>
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'clamp(48px, 6vw, 96px)', alignItems: 'start' }}>
          ...
          <Reveal>
            <div style={{ position: 'relative', aspectRatio: '3/4', maxWidth: 380 }}>
              ...
              <div style={{ position: 'absolute', inset: 0, background: C.bgCard, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <Reveal delay={0.15}>
            <div>
              <p style={{ fontFamily: 'var(--font-outfit)', fontSize: 15, fontWeight: 300, color: C.mutedLight, lineHeight: 1.85, marginBottom: 20 }}>
              ...
              <p style={{ fontFamily: 'var(--font-outfit)', fontSize: 15, fontWeight: 300, color: C.mutedLight, lineHeight: 1.85, marginBottom: 32 }}>
                Co-fundé NorteIA junto a Luis Salgado con un objetivo claro: que las pymes españolas puedan adoptar la IA con confianza, cumpliendo el EU AI Act y sin perder de vista sus objetivos de negocio.
              </p>

              {/* Valores */}
              <p style={{ fontFamily: 'var(--font-outfit)', fontSize: 11, fontWeight: 500, letterSpacing: '0.24em', textTransform: 'uppercase', color: C.copper, marginBottom: 20 }}>Valores</p>
              {[
                { v: 'Honestidad', d: 'Digo lo que la IA puede hacer y lo que no. Sin hype innecesario.' },
                { v: 'Practicidad', d: 'Todo lo que enseño parte de casos de uso reales, no de teoría.' },
                { v: 'Ética en IA', d: 'La tecnología debe servir a las personas, no al revés.' },
              ].map((item) => (
                <div key={item.v} style={{ display: 'flex', gap: 16, marginBottom: 20 }}>
                  <div style={{ width: 4, height: 4, background: C.copper, borderRadius: '50%', marginTop: 8, flexShrink: 0 }} />
                  <div>
                    <p style={{ fontFamily: 'var(--font-outfit)', fontSize: 13, fontWeight: 500, color: C.white, marginBottom: 4, letterSpacing: '0.04em' }}>{item.v}</p>
              ...
              <div style={{ display: 'flex', gap: 16, marginTop: 36, flexWrap: 'wrap' }}>
                <a href="https://www.linkedin.com/in/victormagoheredia/" target="_blank" rel="noopener noreferrer" style={{ fontFamily: 'var(--font-outfit)', fontSize: 12, fontWeight: 500, letterSpacing: '0.16em', textTransform: 'uppercase', background: 'transparent', color: C.copper, border: `1px solid ${C.borderCu}`, padding: '11px 24px', textDecoration: 'none' }}>LinkedIn →</a>
                <a href="https://norteia.es" target="_blank" rel="noopener noreferrer" style={{ fontFamily: 'var(--font-outfit)', fontSize: 12, fontWeight: 400, letterSpacing: '0.16em', textTransform: 'uppercase', color: C.mutedLight, textDecoration: 'none', display: 'flex', alignItems: 'center' }}>NorteIA →</a>
              </div>
            </div>
          </Reveal>
        </div>
        ...
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          <Reveal>
            <div style={{ marginBottom: 56, textAlign: 'center' }}>
              ...
              <h2 style={{ fontFamily: 'var(--font-cormorant)', fontSize: 'clamp(28px, 3.5vw, 44px)', fontWeight: 400, color: C.white }}>
            ...
            {timeline.map((item, i) => (
              <Reveal key={item.year} delay={i * 0.05}>
                <div style={{ position: 'relative', marginBottom: 36 }}>
                  {/* Dot */}
                  <div style={{ position: 'absolute', left: -36, top: 4, width: 8, height: 8, borderRadius: '50%', background: i === timeline.length - 1 ? C.orange : C.copper, border: `2px solid ${C.bg}` }} />
      ...
      <section style={{ padding: 'clamp(60px, 8vw, 100px) clamp(24px, 5vw, 64px)', textAlign: 'center' }}>
        <Reveal>
          <div style={{ maxWidth: 540, margin: '0 auto' }}>
            <h2 style={{ fontFamily: 'var(--font-cormorant)', fontSize: 'clamp(28px, 3.5vw, 44px)', fontWeight: 300, color: C.white, marginBottom: 20 }}>
              ¿Hablamos sobre tu empresa<br /><em style={{ color: C.copper, fontStyle: 'italic' }}>y la IA?</em>
            </h2>
            <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap', marginTop: 32 }}>
              <Link href="/webinar" style={{ fontFamily: 'var(--font-outfit)', fontSize: 12, fontWeight: 500, letterSpacing: '0.16em', textTransform: 'uppercase', background: C.orange, color: '#fff', padding: '14px 32px', textDecoration: 'none' }}>Webinar gratuito</Link>
